# TODO
# Add ability to name the venn diagram
# What to do with "Other Columns"?
# Add function to install packages automatically
# Get a better name than data1 and 2
...
filter_fuzzylogic_df <- fuzzylogic_df %>%
  select(ID, ID2, distance) %>% #, othercolumns) %>%
...
fuzzyLogicExcel <- function(file = "default", col.x = "default", 
...
#Import the data
data1 <- read_xlsx(file, col_names = TRUE, sheet = 1)
data2 <- read_xlsx(file, col_names = TRUE, sheet = 2)

results <- fuzzyLogic(data1 = data1, data2 = data2, col.x = col.x, 
                      col.y = col.y, fuzziness = fuzziness, error = error, 
                      othercolumns = othercolumns)

#Create a new tab on the file
if (append == TRUE) {
    write.xlsx(results$filter_fuzzylogic_df, file = file,
               addWorksheet(sheetName = "FinalReport"), append = TRUE)
}
